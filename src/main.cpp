...
unsigned int nTargetSpacing = 1 * 60; // 1 min
unsigned int nStakeMinAge = 2 * 365 * 24 * 60 * 60;  //disabled
unsigned int nStakeMaxAge = 2 * 365 * 24 * 60 * 61; //disabled
...
const string strMessageMagic = "ARTA Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	{ 
		nSubsidy = 240000000 * COIN; return nSubsidy; 
	}
    else if(pindexBest->nHeight+1 > 1)
	{
        nSubsidy = 12 * COIN;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE;    // its 0 disabled
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ARTA", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "ARTA 2017";
        CTransaction txNew;
        txNew.nTime = 1490980633;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1490980633;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce = 2113949;
		if(fTestNet)
        {
            block.nNonce = 0 ;
        }
		
		uint256 nhashMerkleRoot = uint256("0x77120fb2854fb261841c76610ad04296686d190e2df5b3eec9cd2f943482623a");
...
unsigned char pchMessageStart[4] = { 0xee, 0xa2, 0xea, 0xd7 };
